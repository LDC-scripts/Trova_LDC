...
// @updateURL    https://raw.githubusercontent.com/LDC-scripts/Trova_LDC/main/Trova_LDC_v15.04.user.js
// @downloadURL  https://raw.githubusercontent.com/LDC-scripts/Trova_LDC/main/Trova_LDC_v15.04.user.js
// @run-at       document-end
// ==/UserScript==
...
(function () {
    ...
    // ============================================================
    function getCSVUrl() {
        // Prima controlla se c'è un valore salvato, poi usa il default preconfigurato
        return GM_getValue('csv_url', CSV_URL_DEFAULT);
    }

    function chiediCSVUrl(isUpdate) {
        const attuale = getCSVUrl();
        const messaggio = isUpdate
            ? '🔑 Trova LDC — Modifica link CSV\n\nLink attuale:\n' + (attuale || '(non configurato)') + '\n\nInserisci il nuovo link CSV:'
            : '🔑 Trova LDC — Configurazione iniziale\n\nInserisci il link CSV del foglio Database_Chiavi:\n(verrà salvato localmente sul tuo browser)';

        const url = prompt(messaggio, attuale || '');

        if (url === null) return null; // utente ha annullato

        if (url.trim().startsWith('https://')) {
            GM_setValue('csv_url', url.trim());
            alert('✓ Link CSV salvato correttamente.\n\nRicarica la pagina per applicare le modifiche.');
            return url.trim();
        } else {
    ...
    function verificaConfigurazione() {
        const url = getCSVUrl();
        if (url) {
            alert('✓ Trova LDC v15.04 — Configurazione attiva\n\nLink CSV:\n' + url + '\n\nRighe caricate: ' + db.rows.length);
        } else {
            alert('⚠ Trova LDC v15.04 — Nessun link CSV configurato.\n\nClicca "Configura link CSV" per inserirlo.');
        }
    }

    // ============================================================
    // MENU TAMPERMONKEY (visibile cliccando l'icona in toolbar)
    // ============================================================
    GM_registerMenuCommand('⚙ Configura link CSV', () => {
        chiediCSVUrl(true);
    });

    GM_registerMenuCommand('✓ Verifica configurazione', () => {
    ...
    function inizializza() {
        let url = getCSVUrl();
        if (!url) {
            // Primo avvio: chiede il link
            url = chiediCSVUrl(false);
            if (!url) {
                console.warn('Trova LDC v15.04: nessun link CSV configurato. Clicca sull\'icona Tampermonkey per configurarlo.');
                return;
            }
        }
    ...
    function caricaCSV(url, callback) {
        ...
        GM_xmlhttpRequest({
            ...
            onload: function (res) {
                ...
                console.log("Trova LDC v15.04: DB pronto — " + db.rows.length + " righe.");
                if (callback) callback();
            },
            onerror: () => console.error("Trova LDC v15.04: Errore caricamento CSV.")
        });
    }
